...
Server.prototype.start = function( done ) {
  ...
  self._server = telnet.createServer(function(client) {
    ...
    client.addOutput = function(output) {
      client.write('\u001B[' + (client.height - 1) + ';1H' );
    }
    ...
    client.initiateLogin = function() {
      ...
      client.expectInput(function(input) {
        console.log('WE ARE COOKING WITH FIRE', input);

        client.write('\nWelcome, ' + input );
        client.write('\nEnter a password:');
        client.write('\n> ');
        client.expectInput(function(password) {
          console.log('password:', password);
        });
      });
    }
    
    client.expectInput = function(func) {
      client.removeAllListeners('input');
      client.on('input', func);
    }
    ...
    client.on('data', function(b) {
      ...
      if (hex === '7f') {
        ...
        console.log('input buffer now:', client.inputBuffer);
